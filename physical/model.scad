...
DOWEL_PRINTED_SOCKET_BLOCK = DOWEL_PRINTED_SOCKET_RADIUS * 4;
// Depth of a socket to accept the dowel rod
DOWEL_PRINTED_SOCKET_DEPTH = DOWEL_PRINTED_SOCKET_RADIUS * 4;
...
// Bearing dimensions
BEARING_INNER_RADIUS = 27/2; // Inner radius
...
//
// The center of the spoke holes are at BEARING_THICKNESS + BEARING_INNER_LIP +
...
module axel(num_spokes) {
	...
	inner_radius = max(min_inner_radius, BEARING_INNER_RADIUS + BEARING_INNER_LIP - BEARING_PRINT_SLACK);
	...
	color(COLOUR_3D_PRINTED) {
		...
		// Axel body
		translate([0,0,BEARING_THICKNESS + BEARING_INNER_LIP])
		difference() {
			union() {
				// Central cylinder
